<?php
/*********************************************************************************
 * The contents of this file are subject to the SugarCRM Public License Version 1.1.2
 * ("License"); You may not use this file except in compliance with the
 * License. You may obtain a copy of the License at http://www.sugarcrm.com/SPL
 * Software distributed under the License is distributed on an  "AS IS"  basis,
 * WITHOUT WARRANTY OF ANY KIND, either express or implied. See the License for
 * the specific language governing rights and limitations under the License.
 * The Original Code is:  SugarCRM Open Source
 * The Initial Developer of the Original Code is SugarCRM, Inc.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.;
 * All Rights Reserved.
 * Contributor(s): ______________________________________.
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/vtiger_crm/sugarcrm/modules/Contacts/language/en_us.lang.php,v 1.14 2005/03/24 17:47:43 rank Exp $
 * Description:  Defines the Romanian language pack
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): Magister Software SRL, Bucharest, Romania, www.magister.ro
 ********************************************************************************/

$mod_strings = Array(
// Mike Crowe Mod --------------------------------------------------------Added for general search
'LBL_MODULE_NAME'=>'Contacte',
'LBL_INVITEE'=>'Rapoarte directe',
'LBL_MODULE_TITLE'=>'Contacte: Index',
'LBL_SEARCH_FORM_TITLE'=>'Cauta contact',
'LBL_LIST_FORM_TITLE'=>'Lista contacte',
'LBL_NEW_FORM_TITLE'=>'Contact nou',
'LBL_CONTACT_OPP_FORM_TITLE'=>'Contact-oportunitate:',
'LBL_CONTACT'=>'Contact:',

'LBL_LIST_NAME'=>'Nume',
'LBL_LIST_LAST_NAME'=>'Nume Familie',
'LBL_LIST_FIRST_NAME'=>'Prenume',
'LBL_LIST_CONTACT_NAME'=>'Nume Contact',
'LBL_LIST_TITLE'=>'Titlu',
'LBL_LIST_ACCOUNT_NAME'=>'Nume Cont',
'LBL_LIST_EMAIL_ADDRESS'=>'Email',
'LBL_LIST_PHONE'=>'Nr tel',
'LBL_LIST_CONTACT_ROLE'=>'Rol',

//DON'T CONVERT THESE THEY ARE MAPPINGS
'db_last_name' => 'LBL_LIST_LAST_NAME',
'db_first_name' => 'LBL_LIST_FIRST_NAME',
'db_title' => 'LBL_LIST_TITLE',
'db_email1' => 'LBL_LIST_EMAIL_ADDRESS',
'db_email2' => 'LBL_LIST_EMAIL_ADDRESS',
//END DON'T CONVERT
	
'LBL_EXISTING_CONTACT' => 'Ati utilizat un contact existent',
'LBL_CREATED_CONTACT' => 'Ati creat un contact nou',
'LBL_EXISTING_ACCOUNT' => 'Ati utilizat un cont vtiger existent',
'LBL_CREATED_ACCOUNT' => 'Ati creat un cont vtiger nou',
'LBL_CREATED_CALL' => 'Ati creat un apel nou',
'LBL_CREATED_MEETING' => 'Ati creat o intalnire noua',
'LBL_ADDMORE_BUSINESSCARD' =>'Adauga carte de vizita',

'LBL_BUSINESSCARD' => 'Carte de vizita',

'LBL_NAME'=>'Nume:',
'LBL_CONTACT_NAME'=>'Nume Contact:',
'LBL_CONTACT_INFORMATION'=>'Info Contact',
'LBL_CUSTOM_INFORMATION'=>'Info Personalizata',
'LBL_FIRST_NAME'=>'Nume Familie:',
'LBL_OFFICE_PHONE'=>'Nr tel birou:',
'LBL_ACCOUNT_NAME'=>'Nume Cont:',
'LBL_ANY_PHONE'=>'Orice nr tel:',
'LBL_PHONE'=>'Nr tel:',
'LBL_LAST_NAME'=>'Nume Familie:',
'LBL_MOBILE_PHONE'=>'Nr tel mobil:',
'LBL_HOME_PHONE'=>'Nr tel acasa:',
'LBL_LEAD_SOURCE'=>'Prospectare:',
'LBL_OTHER_PHONE'=>'Alt nr tel:',
'LBL_FAX_PHONE'=>'Nr fax:',
'LBL_TITLE'=>'Titlu:',
'LBL_DEPARTMENT'=>'Departament:',
'LBL_BIRTHDATE'=>'Data Nastere:',
'LBL_EMAIL_ADDRESS'=>'Email:',
'LBL_OTHER_EMAIL_ADDRESS'=>'Alt email:',
'LBL_ANY_EMAIL'=>'Orice email:',
'LBL_REPORTS_TO'=>'Raporteaza catre:',
'LBL_ASSISTANT'=>'Asistent:',
'LBL_YAHOO_ID'=>'Yahoo! ID:',
'LBL_ASSISTANT_PHONE'=>'Nr tel asistent:',
'LBL_DO_NOT_CALL'=>'Apel interzis:',
'LBL_EMAIL_OPT_OUT'=>'Email optional:',
'LBL_PRIMARY_ADDRESS'=>'Adresa principala:',
'LBL_ALTERNATE_ADDRESS'=>'Alta adresa:',
'LBL_ANY_ADDRESS'=>'Orice adresa:',
'LBL_CITY'=>'Oras:',
'LBL_STATE'=>'Judet:',
'LBL_POSTAL_CODE'=>'Cod postal:',
'LBL_COUNTRY'=>'Tara:',
'LBL_DESCRIPTION_INFORMATION'=>'Info descriere',
'LBL_IMAGE_INFORMATION'=>'Info imagine contact:',
'LBL_ADDRESS_INFORMATION'=>'Info adresa',
'LBL_DESCRIPTION'=>'Descriere:',
'LBL_CONTACT_ROLE'=>'Rol:',
'LBL_OPP_NAME'=>'Nume oportunitate:',
'LBL_DUPLICATE'=>'Potential contact duplicat',
'MSG_DUPLICATE' => 'Crearea acestui contact poate genera un contact duplicat. Selectati un contact din lista de mai jos sau click pe Creaza contact nou pentru a continua crearea unui contact continand datele introduse anterior.',

'LNK_NEW_APPOINTMENT' => 'Intalnire noua',
'LBL_ADD_BUSINESSCARD' => 'Adauga carte de vizita',
'NTC_DELETE_CONFIRMATION'=>'Sunteti sigur ca doriti sa stergeti aceasta inregistrare?',
'NTC_REMOVE_CONFIRMATION'=>'Sunteti sigur ca doriti sa stergeti acest contact?',
'NTC_REMOVE_DIRECT_REPORT_CONFIRMATION'=>'Sunteti sigur ca doriti sa stergeti aceasta inregistrare in calitate de vtiger_report direct?',
'ERR_DELETE_RECORD'=>"Trebuie sa specificati un nr de ordine pentru a sterge acest contact.",
'NTC_COPY_PRIMARY_ADDRESS'=>'Copiaza adresa principala la adresa alternativa',
'NTC_COPY_ALTERNATE_ADDRESS'=>'Copiaza adresa alternativa la adresa principala',

'LBL_SELECT_CONTACT'=>'Selecteaza contact',
//Added for search heading
'LBL_GENERAL_INFORMATION'=>'Info generala',



//for v4 release added
'LBL_NEW_POTENTIAL'=>'Potential nou',
'LBL_POTENTIAL_TITLE'=>'Potentiale',

'LBL_NEW_TASK'=>'Task nou',
'LBL_TASK_TITLE'=>'Task-uri',
'LBL_NEW_CALL'=>'Apel nou',
'LBL_CALL_TITLE'=>'Apeluri',
'LBL_NEW_MEETING'=>'Intalnire noua',
'LBL_MEETING_TITLE'=>'Intalniri',
'LBL_NEW_EMAIL'=>'Email nou',
'LBL_EMAIL_TITLE'=>'Email-uri',
'LBL_NEW_NOTE'=>'Nota noua',
'LBL_NOTE_TITLE'=>'Note',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'Instrumente contact',

'Salutation'=>'Salutare',
'First Name'=>'Prenume',
'Office Phone'=>'Nr tel birou',
'Last Name'=>'Nume Familie',
'Mobile'=>'Nr tel mobil',
'Account Name'=>'Nume Cont',
'Home Phone'=>'Nr tel acasa',
'Lead Source'=>'Prospectare',
'Other Phone'=>'Alt nr tel',
'Title'=>'Titlu',
'Fax'=>'Nr fax',
'Department'=>'Departament',
'Birthdate'=>'Data nastere',
'Email'=>'Email',
'Reports To'=>'Raporteaza catre',
'Assistant'=>'Asistent',
'Yahoo Id'=>'ID Yahoo',
'Assistant Phone'=>'Nr tel asistent',
'Do Not Call'=>'Apel interzis',
'Email Opt Out'=>'Email optional',
'Assigned To'=>'Asignat la',
'Campaign Source'=>'Sursa campanie',
'Reference' =>'Referinta',
'Created Time'=>'Creat la ora',
'Modified Time'=>'Modificat la ora',
'Mailing Street'=>'Strada',
'Other Street'=>'Alta strada',
'Mailing City'=>'Oras',
'Mailing State'=>'Judet',
'Mailing Zip'=>'Cod postal',
'Mailing Country'=>'Tara',
'Mailing Po Box'=>'Cutie postala',
'Other Po Box'=>'Alta cutie postala',
'Other City'=>'Alt oras',
'Other State'=>'Alt judet',
'Other Zip'=>'Alt cod postal',
'Other Country'=>'Alta tara',
'Contact Image'=>'Imagine contact',
'Description'=>'Descriere',

// Added vtiger_fields for Add Business Card
'LBL_NEW_CONTACT'=>'Contact nou',
'LBL_NEW_ACCOUNT'=>'Cont nou',
'LBL_NOTE_SUBJECT'=>'Noteaza subiect:',
'LBL_NOTE'=>'Nota:',
'LBL_WEBSITE'=>'Site Web:',
'LBL_NEW_APPOINTMENT'=>'Intalnire noua',
'LBL_SUBJECT'=>'Subiect:',
'LBL_START_DATE'=>'Data inceput:',
'LBL_START_TIME'=>'Ora inceput:',

//Added vtiger_field after 4_0_1
'Portal User'=>'Utilizator Portal',
'LBL_CUSTOMER_PORTAL_INFORMATION'=>'Info portal clienti',
'Support Start Date'=>'Data inceput asistenta',
'Support End Date'=>'Data sfarsit asistenta',
//Added for 4.2 Release -- CustomView
'Name'=>'Nume',
'LBL_ALL'=>'Toate',
'LBL_MAXIMUM_LIMIT_ERROR'=>'Ne pare rau, fisierul incarcat depaseste limita maxima admisa. Va rugam incercati cu un fisier mai mic de 800000 bytes',
'LBL_UPLOAD_ERROR'=>'Probleme la incarcarea fisierului. Incercati din nou!',
'LBL_IMAGE_ERROR'=>'Fisierul nu are extensia de imagine(.gif/.jpg/.png)',
'LBL_INVALID_IMAGE'=>'Fisier invalid sau fisierul nu are date',

//Added after 5Alpha5
'Notify Owner'=>'Notifica proprietar',

//Added for Picklist Values
'--None--'=>'--Nimic--',

'Mr.'=>'D-nul',
'Ms.'=>'D-na',
'Mrs.'=>'D-ra',
'Dr.'=>'Dr.',
'Prof.'=>'Prof.',

'Cold Call'=>'Apel Rece',
'Existing Customer'=>'Client existent',
'Self Generated'=>'Autogenerat',
'Employee'=>'Angajat',
'Partner'=>'Partener',
'Public Relations'=>'Relatii publice',
'Direct Mail'=>'Posta directa',
'Conference'=>'Conferinta',
'Trade Show'=>'Expozitie',
'Web Site'=>'Site Web',
'Word of mouth'=>'Recomandare verbala',
'Other'=>'Altul',
'User List'=>'Lista utilizatori',

//Added for 5.0.3
'Customer Portal Login Details'=>'Detalii autentificare portal client',
'Dear'=>'Stimate',
'Your Customer Portal Login details are given below:'=>'Detaliile dvs. de autentificare pe portalul pentru clienti sunt urmatoarele:',
'User Id :'=>'ID Utilizator: ',
'Password :'=>'Parola: ',
'Please Login Here'=>'Autentificare aici',
'Note :'=>'Nota:',
'We suggest you to change your password after logging in first time'=>'Este recomandat sa modificati parola dupa prima autentificare.',
'Support Team'=>'Serviciul de asistenta nationala certificata, Magister Software',

'TITLE_AJAX_CSS_POPUP_CHAT'=>'Ajax Css-Popup Chat',

// Added after 5.0.4 GA

// Module Sequence Numbering
'Contact Id' => 'ID Contact',
// END
);

?>
